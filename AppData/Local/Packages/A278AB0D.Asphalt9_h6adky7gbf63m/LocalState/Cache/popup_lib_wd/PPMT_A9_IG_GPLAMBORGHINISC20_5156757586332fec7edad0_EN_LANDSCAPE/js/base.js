...
function scaleAreas()
{
    ...
    for(var i = 0; i < arrayAreas.length; i++)
    {
        ...
        if(arrayAreas[i].className.indexOf(" discount") > -1) {
			generateDiscount(rarray);
        } else if(arrayAreas[i].href !== 'exit:') {
...
var touchAnimation = (function(){
    ...
    var init = function() {
        image = document.querySelector('#imgContainer img');
        aa = document.querySelector('map').areas;
        for (var i =0; i < aa.length; i++) {
            if (aa[i].getAttribute("effect")) {
                areas.push(aa[i]);
            }
        }
        if (areas.length === 0) {
        ...
        if('ontouchstart' in window) {
            touchSupported = true;
            document.body.addEventListener('touchstart', touchOn, true);
            document.body.addEventListener('touchend', touchOff, true);
        } else {
            document.body.addEventListener('mousedown', touchOn, true);
            document.body.addEventListener('mouseup', touchOff, true);
        }
    };
    ...
    var findTouchedArea = function (e) {
        var t = getTouchPos(e);
        var r = image.getBoundingClientRect();
        for (var i =0; i < areas.length; i++) {
            var crd  = areas[i].coords.split(',');
            var x1 = parseInt(crd[0]) + r.x;
            var y1 = parseInt(crd[1]) + r.y;
            var x2 = parseInt(crd[2]) + r.x;
            var y2 = parseInt(crd[3]) + r.y;
    ...
    var touchOn = function(e) {
        var a = findTouchedArea(e);
        if (!a || animations[a.idx]) {
             return;
        }
        var d = document.createElement('div');
        d.className = 'animation-div';
        d.style.left = a.x1 + 'px';
        d.style.top = a.y1 + 'px';
        d.style.width = (a.x2 - a.x1) + 'px';
        d.style.height = (a.y2 - a.y1) + 'px';
        document.body.appendChild(d);
        animations[a.idx] = d;
    };

    var touchOff = function(e) {
        var a = findTouchedArea(e);
        if (!a) {
            return;
        }
        navigator.vibrate(100);
        var el = animations[a.idx];
        if (el) {
            animations[a.idx] = null;
...
function adjustSplashScreen()
{
    ...
    window.scrollTo(0,0);
            ...
            if(WEBVIEW_RECEIVED_OS == 'WP8' || WEBVIEW_RECEIVED_OS == 'windows' || WEBVIEW_RECEIVED_OS == 'steam')
        {
